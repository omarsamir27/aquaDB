...
impl DatabaseServer {
    pub fn new(home_dir: &str, addr: Vec<String>) -> Self {
        let sockets = addr.iter().map(|ip| { let socket = TcpListener::bind(ip).expect(format!("Could not bind to {ip}").as_str());  socket.set_nonblocking(true); socket })
        .collect();
        Self { home_dir: home_dir.to_string(), sockets, bind_addr: addr }
    }
    pub fn run(&self) {
