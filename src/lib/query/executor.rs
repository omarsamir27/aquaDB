...
impl<'db> Executor<'db> {
    ...
    pub fn execute_seqscan(&mut self, node: SeqScan) {
        ...
        let headers = tblmgr
            .get_layout().type_map().into_iter().filter(|(k,_)| fields.contains(k)).collect();
...
fn construct_fromBytes(data:&[u8],t:Type) -> Box<dyn FromBytes>{
    match t {
        ...
        Type::Character(char) => Box::new(String::from_utf8(data.to_vec()).unwrap()) as Box<dyn FromBytes>
    }
}