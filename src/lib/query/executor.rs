...
impl<'db> Executor<'db> {
    ...
    pub fn insert_record(&mut self, record: Record, schema: Schema) -> Result<(), String> {
        ...
        let mut need_heapscan = unique_fields.clone().any(|(k,v)| !target_table.field_has_index(k) );
        let unique_field_names = unique_fields.clone().map(|(k,v)| k).collect::<HashSet<_>>();
        let mut unique_record_fields = record.clone().into_iter().map(|(name,data)| (name,data)).collect::<HashMap<_,_>>();
        ...
        if need_heapscan{
            ...
            let duplicate_by_hash = unique_fields.clone()
                .filter_map(|(f,_)|
                   target_table.hashscan_iter(f).map(|idx| (f,idx))
                    )
                .any(|(name, mut hash)| {
            ...
            let duplicate_by_tree = unique_fields
                .filter_map(|(f,_)|
                    target_table.btree_iter(f,evalexpr::Operator::Eq).map(|idx| (f,idx))
                )
                .any(|(name, mut tree)| {
                    dup_val = name.to_string();
        ...
        let foreign_referring = fields
            .iter()
            .filter(|(_, v)| v.foreign_reference().is_some()).collect::<HashMap<_,_>>();

        let mut foreign_record_fields = record.clone().into_iter().map(|(name,data)| (name,data)).collect::<HashMap<_,_>>();
        ...
        for (name,field) in foreign_referring{
            ...
            else if let Some(mut btree) = ref_table_mgr.btree_iter(ref_col,evalexpr::Operator::Eq){
        ...
        if ok_insert{
            let target_table = self
                .db_tables
                .get_mut(schema.name()).unwrap();
            target_table.try_insert_tuple(record);
            Ok(())
        }
        else {
            Err(format!("Foreign Reference Not Found : ({}.{})",foreign_not_found.0,foreign_not_found.1))
        }
