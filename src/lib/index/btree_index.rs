...
impl NodePage {
    ...
    fn get_greater_than_or_equal(&self, key: Vec<u8>) -> Option<Vec<Rid>> {
        match self {
            ...
            NodePage::Leaf(node) => node.get_greater_than_or_equal(key),
        }
    }

    fn get_less_than_or_equal(&self, key: Vec<u8>) -> Option<Vec<Rid>> {
        match self {
            NodePage::Internal(node) => node.get_less_than_or_equal(key),
            NodePage::Leaf(node) => node.get_less_than_or_equal(key),
        }
    }
            ...
            if last_index == self.heap_page.tuple_pointers.len() as u16 - 1 {
                ...
                while next_block_num != 0 {
                    ...
                    let next_frame = self
                        ...
                        .unwrap();
                    let next_heap =
                        HeapPage::new(next_frame.clone(), &block_id, self.leaf_layout.clone());
